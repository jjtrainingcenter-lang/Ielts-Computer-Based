...
/**
 * Small source-level UI cleanup for legacy components that are otherwise very
 * large to duplicate safely. Google Drive links are normalized by the runtime
 * media components; these edits make the Admin inputs/previews communicate and
 * preview that support correctly, and remove the duplicate Task 0/150 badge.
 */
const mediaAndWritingUiPlugin = (): Plugin => ({
    ...
    if (cleanId.endsWith('/src/components/WritingEditor.tsx')) {
      ...
      if (markerIndex !== -1) {
        ...
        if (spanStart !== -1 && spanEndStart !== -1) {
          const spanEnd = spanEndStart + '</span>'.length;
          next = next.slice(0, spanStart) + next.slice(spanEnd);
        }
      }
