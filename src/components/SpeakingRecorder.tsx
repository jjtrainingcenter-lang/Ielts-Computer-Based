...
import { Mic, MicOff, Square, Play, Pause, Clock, Sparkles, AlertCircle, CheckCircle, RotateCcw } from 'lucide-react';
...
export const SpeakingRecorder: React.FC<SpeakingRecorderProps> = ({
  ...
  const currentTask = tasks.find((t) => t.partNumber === activePart) || tasks[1];
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full bg-[#F4F7F9] border-t border-gray-300">
      ...
      <div className="lg:w-1/2 p-6 overflow-y-auto border-r border-gray-300 space-y-6 bg-white">
        ...
        <div className="flex items-center space-x-2 border-b border-gray-200 pb-4">
          {[1, 2, 3].map((pNum) => (
            <button
              key={pNum}
              type="button"
              onClick={() => setActivePart(pNum as 1 | 2 | 3)}
        ...
        {/* Part 2 Cue Card Display Box */}
        {activePart === 2 && currentTask.cueCard && (
          <div className="p-6 bg-amber-50/70 rounded border border-amber-200 space-y-4 shadow-xs">
            ...
            <ul className="space-y-2 text-xs text-gray-800 pl-4 list-disc">
              {currentTask.cueCard.bulletPoints.map((bp, i) => (
                <li key={i}>{bp}</li>
              ))}
            </ul>
          </div>
        ...
        {/* Part 1 or Part 3 Questions List */}
        {activePart !== 2 && (
          <div className="space-y-3">
            ...
            <div className="space-y-2.5">
              {currentTask.questions.map((q, idx) => (
                ...
                >
                  <span className="w-5 h-5 rounded bg-[#214162] text-white font-bold text-xs flex items-center justify-center shrink-0">
                    {idx + 1}
                  </span>
                  <p>{q}</p>
                </div>
        ...
        {/* Candidate Scratchpad / Prep Notes */}
        <div className="space-y-2 pt-2">
          <label className="text-xs font-bold uppercase tracking-widest text-gray-500 block">
            Candidate Notes / Scratchpad (1 min preparation):
          </label>
          <textarea
            value={candidateNotes}
            onChange={(e) => setCandidateNotes(e.target.value)}
            placeholder="Jot down key points, vocabulary, or structured bullet points during preparation..."
      ...
      {/* Right Pane: Preparation Ring & Recorder Controls */}
      <div className="lg:w-1/2 p-6 flex flex-col justify-between bg-white">
        <div className="space-y-6">
          {/* Prep Timer Card for Part 2 */}
          {activePart === 2 && (
            <div className="p-5 bg-gray-50 rounded border border-gray-300 flex items-center justify-between">
              ...
              <div className="flex items-center space-x-3">
                <span className="font-mono text-xl font-bold text-[#214162]">
                  00:{String(prepTimeLeft).padStart(2, '0')}
                </span>
                <button
                  type="button"
                  onClick={() => setIsPrepActive(!isPrepActive)}
                  className={`px-3 py-1.5 rounded text-xs font-semibold text-white transition-colors ${
                    isPrepActive ? 'bg-amber-600 hover:bg-amber-500' : 'bg-[#214162] hover:bg-[#2b547e]'
                  }`}
                >
                  {isPrepActive ? 'Pause' : 'Start Prep'}
                </button>
              </div>
            </div>
          )}

          {/* Voice Microphone Recorder Console */}
            ...
            <div className="relative">
              <div
                className={`w-20 h-20 rounded-full flex items-center justify-center transition-transform ${
                  isRecording
                    ? 'bg-red-600 animate-pulse ring-8 ring-red-400/30'
                    : 'bg-blue-600 ring-4 ring-white/20'
                }`}
              >
            ...
            <div className="flex items-center space-x-3">
              {!isRecording ? (
                <button
                  ...
                  className="px-5 py-2.5 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded shadow-xs transition-all flex items-center space-x-2"
                >
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white font-bold text-xs rounded border border-white/20 transition-all flex items-center space-x-2"
                >
              ...
              {audioUrl && (
                <button
                  ...
                  className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded shadow-xs transition-all flex items-center space-x-2"
                >
